...
def _agent_yaml(slug: str, name: str, with_solution: bool = False) -> str:
    ...
    return (
        ...
        f"name: {name}\n"
        "description: >-\n"
        "  TODO: describe what this agent does for users. Include the workflow it owns\n"
...
def _solution_yaml(slug: str, name: str, tagline: str, solution_id: str) -> str:
    ...
    return (
        ...
        "#\n"
        "# `templates:` is the canonical multi-template list. Even single-\n"
        "# template bundles use list form. To split out atomic tools/routines\n"
        "# so they can be installed standalone alongside the deployable\n"
        "# agent, add `kind: AgentToolTemplate` / `kind: AgentRoutineTemplate`\n"
        "# YAML files under tools/ or routines/ and add `- template_path:`\n"
        "# entries here. Then reference them from the AgentTemplate body's\n"
        "# `tools:` / `routines:` via the polymorphic inline shape:\n"
        "#\n"
        "#   tools:\n"
        "#     - tool_type: builtin              # inline literal\n"
        "#       builtin_tool_key: knowledge_search\n"
        "#     - template_path: tools/query-osv.yaml   # path-mode\n"
        "#     - template_ref: query-osv              # by lookup_key\n"
        "#\n"
        "# Each atomic template carries its own `setup_requirements:` block —\n"
        "# users installing just one tool/routine pick up the env vars +\n"
        "# installs it depends on.\n"
        "\n"
        "kind: Solution\n"
