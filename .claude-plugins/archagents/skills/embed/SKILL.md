---
name: embed
description: Use when the user wants to embed an ArchAgent agent into Claude Code, asks about the active embed state, wants to refresh or stop the embed, or refers to working as a specific ArchAgent agent inside Claude Code. Trigger phrases include "embed agent", "act as this agent", "be this agent", "start embed", "sync embed", "stop embed", "which agent is embedded", and "use the active agent identity".
allowed-tools: ["Bash(archagent:*)"]
---

# ArchAgent Embed

Manage ArchAgent embed sessions through the ArchAgent CLI and keep the Claude Code session aligned with the active identity file.
...
- If the CLI reports an auth or app error, route to `/archagents:auth` or suggest `--app <id>`.
