...
1) Read agents/cross-org-collab-agent/agent.yaml — the `setup_requirements:` block lists everything you'll need.
...
4) Deploy: archagent install agentsample cross-org-collab-agent
...
- **Integration installs** (the GitHub App) link out to the install
  URL — install it on the private repo the agent reads from.
- **Custom verifiers** (e.g. confirming the bot can read
  `REPO_OWNER/REPO_NAME`) re-run on a daily sweep, so a deleted
